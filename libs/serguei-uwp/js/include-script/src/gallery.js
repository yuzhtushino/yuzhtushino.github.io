...
(function (root, document) {
	...
	root.runGallery = function () {
		...
		var manageLightGallery = function (containerClass) {
		...
		var addMacyItems = function (macy, callback) {
			...
			for (i = 0, l = macyItems.length; i < l; i += 1) {
				html.push('<a href="' + macyItems[i].href + '" class="' + macyItemIsBindedClass + '"  aria-label="Показать картинку"><img src="' + transparentPixel + '" class="' + dataSrcImgClass + '" data-src="' + macyItems[i].src + '" alt="" /></a>\n');
				count++;
