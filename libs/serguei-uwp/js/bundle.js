...
(function(root, document) {
	...
	root.onMacyImagesLoaded = function(macy, callback) {
		...
		onLoad = function onLoad() {
			...
			if (imgCounter === imgLength) {
				cb();
				console.log(
					"onMacyImagesLoaded: loaded " + imgCounter + " images"
				);
			}
		};
		...
		onError = function onError() {
			removeListeners(this);
			console.log(
				"onMacyImagesLoaded: failed to load image: " + this.src
			);
		};
	...
	var setUWPBody = function setUWPBody(attrName, attrValue) {
		document.body.setAttribute(attrName, attrValue);
	};

	var getUWPButtons = function getUWPButtons() {
...
(function(root, document) {
	...
	var run = function run() {
		...
		var switchLayoutType = function switchLayoutType() {
			var arrange = function arrange(mqString, callback) {
				var handleMq = function handleMq(x) {
					if (x.matches) {
						if (callback && "function" === typeof callback) {
							callback();
						}
			...
			var onMatch = function onMatch(attrName, layoutType) {
